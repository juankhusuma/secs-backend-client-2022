    ...
    return (
        <div>
            <div className="flex w-full h-[100vh] items-center justify-center flex-col">
                ...
                <p className="text-red-500 mt-10 mb-3">{error ? `${error}` : ""}</p>
                <form onSubmit={(e) => {
                    e.preventDefault();
                    ...
                    }).then((res) => res.status === 200 ? navigate("/login") : res.json().then(data => setError(data.error.message)));
                }} className="flex flex-col">
                    <div className="px-4 py-2  flex flex-col">
                        <label className="text-gray-700" htmlFor="username">Name</label>
                        <input required className="px-4 py-2 border border-black focus:border-2  flex flex-col" name="username" type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Insert name here..." />
                    </div>
                    <div className="px-4 py-2  flex flex-col">
                        <label className="text-gray-700" htmlFor="username">NIP</label>
                        <input required className="px-4 py-2 border border-black focus:border-2  flex flex-col" name="username" type="text" value={NIP} onChange={(e) => setNIP(e.target.value)} placeholder="Insert NIP here..." />
                    </div>
                    <div className="px-4 py-2  flex flex-col">
                ...
                <p>Got an account? <Link to="/login" className="text-blue-600 underline">login here.</Link></p>
            </div>
        </div>
    )   
}